  ...
  export default {
    ...
    methods: {
      fetchData() {
        ...
        column.search(this.currentPage, 10, this.searchMap).then(response => {
      ...
      handleEdit(id) {
        ...
        this.id = id
        if (this.id !== '') {
          column.findById(this.id).then(response => {
            if (response.flag === true) {
      ...
      handleSave() {
        if (this.id !== '') {
          message.handleShowMessage(column.update(this.id, this.pojo), this)
        } else {
          message.handleShowMessage(column.save(this.pojo), this)
        }
      ...
      handleDelete(id) {
        this.$confirm('确定要删除吗？', '提示', {
          ...
          message.handleShowMessage(column.deleteById(id), this)
        })
      },
      handleExamine(id) {
